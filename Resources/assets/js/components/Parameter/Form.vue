        ...
        <div class="panel panel-default" :class="{'form-loading': isLoading}">
            <div class="panel-body">
                <div class="form-group">
                    ...
                    <select2 :multiple="true" :data="productCategories" v-model="form.categories" id="categories"/>
                </div>
                ...
                <label for="display-as-detail" class="switcher switcher-success m-t-1 m-b-2">
                    <input type="checkbox" id="display-as-detail" v-model="form.display_as_detail">
                    <div class="switcher-indicator">
                        <div class="switcher-yes">YES</div>
                        <div class="switcher-no">NO</div>
                    </div>
                    Display as detail <i>(in single product view)</i>
                </label>

                <div class="form-group" v-if="isTypeRange || form.display_as_range">
                    <label for="postfix">Postfix:</label>
                    <input type="text" id="postfix" class="form-control" v-model="form.postfix">
                </div>
                ...
                <template v-else>
                    <label for="attributes-has-icon" class="switcher switcher-success m-t-1 m-b-2">
                        ...
                        Attributes with icons/images <i>(works only for non-countable parameters)</i>
                    </label>

                    <label for="is-countable" class="switcher switcher-success m-t-1 m-b-2">
                        <input type="checkbox" id="is-countable" v-model="form.is_countable">
                        <div class="switcher-indicator">
                            <div class="switcher-yes">YES</div>
                            <div class="switcher-no">NO</div>
                        </div>
                        Is countable <i>(used when quantity should depend on attributes)</i>
                    </label>

                    <label for="display-as-range" class="switcher switcher-success m-t-1 m-b-2" v-if="!isTypeRange">
                        <input type="checkbox" id="display-as-range" v-model="form.display_as_range">
                        <div class="switcher-indicator">
                            <div class="switcher-yes">YES</div>
                            <div class="switcher-no">NO</div>
                        </div>
                        Display as range <i>(works only for numeric type attributes, like weight, size etc.)</i>
                    </label>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['languages', 'productCategories']),
